        ...
        uploadTask.on('state_changed',
          ...
          async () => {
            try {
              ...
              // Run AI analysis in the background without waiting
              if (file.type.startsWith('image/')) {
                fileToDataURI(file).then(dataUri => {
                  analyzeDocument({ photoDataUri: dataUri }).then(analysisResult => {
                    updateDoc(docRef, { aiAnalysis: analysisResult });
                    toast.success(`AI analysis complete: ${analysisResult.documentType}`);
                  }).catch(aiError => {
              ...
              resolve();
               ...
               reject(new Error(`Failed to save document: ${dbError.message}`));
            }
          }
        );
      });
      ...
      <main className="flex-grow container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="flex justify-between items-center mb-8 gap-4">
          ...
          <div className="flex gap-2">
             <Button onClick={handleGenerateQrForMultiple} disabled={selectedDocs.length === 0 || uploadState.isUploading}>
                <QrCode className="mr-2 h-4 w-4" />
                Share Selected ({selectedDocs.length})
              </Button>
            <Button onClick={() => fileInputRef.current?.click()} disabled={uploadState.isUploading}>
              {uploadState.isUploading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Upload className="mr-2 h-4 w-4" />}
              Upload
            </Button>
          </div>
        ...
        {uploadState.isUploading && (
          <div className="mb-8 p-4 rounded-lg border">
            <div className='flex justify-between items-center'>
                <p className="font-medium mb-2">Uploading: {uploadState.fileName}</p>
                <p className="text-sm text-muted-foreground">{uploadState.progress.toFixed(0)}%</p>
            </div>
            <div className="relative h-2 w-full overflow-hidden rounded-full bg-secondary">
              <div className="h-full w-full flex-1 bg-primary transition-all" style={{ transform: `translateX(-${100 - uploadState.progress}%)` }}/>
            </div>
          </div>
        )}
        ...
        <Card className="glowing-border">
            ...
            <CardContent>
                ...
                {!isLoadingDocuments && documents && documents.length > 0 ? (
                    ...
                    !isLoadingDocuments && <div className="text-center py-12"><FileText className="mx-auto h-12 w-12 text-muted-foreground" /><h3 className="mt-4 text-lg font-semibold">No Documents Found</h3><p className="mt-2 text-sm text-muted-foreground">Click "Upload" to add your first document.</p></div>
                )}
                </div>
            </CardContent>
        </Card>
